...
def bimap_channels(X, W):
    ...
    #     Pi=Pi+bimap(X,W[j])
    # 训练好自编码器后需要取中间拟合的数据，这里X.shape是840，10，10
    # 而在训练时，这里传入的X.shape是 30,1, 10, 10
    ...
    for i in range(batch_size):
        for j in range(channels):
            if (eig_mode == 'eig'):
                ...
                p_cpu = P[i, j].cpu()
                u_cpu, s_cpu, v_cpu = torch.svd(p_cpu)
                U[i, j] = u_cpu.to(P[i, j].device)
                S[i, j] = s_cpu.to(P[i, j].device)
    S_fn = op.fn(S, param)
