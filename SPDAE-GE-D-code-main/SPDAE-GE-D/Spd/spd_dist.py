...
class SPD_Distance:
    ...
    def f_norm_square(self, a, b):
        ...
        if np.array_equal(a, b):  # 如果a和b相等，返回0
            return 0.0
        # 返回Frobenius范数平方除以2
        temp = np.log(a) - np.log(b)
        norm = np.square(np.linalg.norm(temp))
        return norm/2
    ...
    # 投影矩阵度量，计算两个点之间的spd距离
    # def projection_metric_spd(self, a, b):
    #     assert a.shape == b.shape  # 确保a和b形状相同
    #     if np.array_equal(a, b):  # 如果a和b相等，返回0
    #         return 0.0
    #     # 返回Frobenius范数平方除以2
    ...
    def projection_metric_spd(self, a, b):
        assert a.shape == b.shape  # 确保a和b形状相同
        if torch.equal(a, b):  # 如果a和b相等，返回0
            return 0.0
